-- =====================================================================================
-- Legacy to Modern Verethfier System Migration
-- =====================================================================================
-- This migration takes you from the original legacy schema to the complete enhanced
-- dynamic role management system while preserving all existing data.
--
-- FROM: Legacy two-table system (verifier_servers + verifier_users + data)
-- TO:   Enhanced dynamic system with rule-based verification and role tracking
--
-- IMPORTANT: Use this ONLY if you have existing legacy data to migrate.
-- For fresh installations, use: 20250706000002_fresh_install.sql instead
--
-- Features included:
-- - Creates modern verification rules and role tracking tables
-- - Migrates legacy data with server_name and role_name extraction
-- - 72-hour grace period for migrated users
...
CREATE TABLE IF NOT EXISTS public.verifier_rules (
    ...
    role_name text DEFAULT '',  -- Add this missing column
...
BEGIN
    ...
    END IF;        -- Migrate data from verifier_users to verifier_user_roles
        ...
        SELECT DISTINCT
            ...
            -- No user name available in legacy data
            NULL AS user_name,
            -- Get actual server name from verifier_servers table
            COALESCE(vs.name, 'Unknown Server') AS server_name,
            -- Get role name from the servers JSONB value
            COALESCE(server_value::text, 'Legacy Role') AS role_name,
            now() AS created_at,
            now() AS updated_at
...
BEGIN
    RAISE NOTICE '=================================================================';
    RAISE NOTICE 'Verethfier Legacy to Modern Migration Successful!';
    RAISE NOTICE '=================================================================';
